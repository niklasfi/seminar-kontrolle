...
Im Allgemeinen ist es schwer  die Gram'sche Matrix auszurechnen. Die Resolvente bzw. ein Fundamental ist meist nicht einfach zu berechnen.

Also suchen wir ein handlicheres Kriterium. Dafür ziehen wir uns zunächst auf den zeitunabhängigen Fall zurück. Seien also $A$ und $B$ unabhängig von $t$. Zunächst ist nicht klar, ob dann die Kontrollierbarkeit auch völlig zeitunabhängig ist. Schließlich hängt die Lösung von Anfangs- und Endpunkt ab.
...
\begin{Beweis}[zu \ref{Kalman Rang Bedingung}]
 \begin{enumerate}
  \item["`$\Leftarrow$"']
    Da $A$ zeitunabhängig ist können wir die Differentialgleichung leicht lösen und erhalten die Resolvente $R(t_1,t_2)=e^{(t_1-t_2)A} \; \forall  (t_1,t_2) \in [T_0,T_1]^2$
    ...
    \end{align*}
		...
		Bewiesen wird die Kontraposition, also wird angenommen: $\dot x(t) = Ax(t)+Bu(t)$ ist nicht kontrollierbar in $[T_0,T_1]$. Dann ist $\G$ nicht invertierbar und hat somit einen nicht trivialen Kern. Es exisitert also $y \in \R^n \setminus \{0\}$ mit $\G y=0$.
    Da Multiplizeren der 0 nichts ändert gilt auch
     \[
	y^T \G y = 0 
     \]
     und somit
    \[ 
      0 = y^T \G y = \int\limits_{T_0}^{T_1} \underbrace{|B^T e^{(T_1-\tau)A^T} y |^2}_{ \geq 0} d\tau.
    \]
      Aus der Stetigkeit von $A,B$ und damit der Stetigkeit von $|B^T e^{(T_1-\tau)A^T} y |^2$ folgt dann 
     \[  
    ...
    Es ist 
    \begin{align*}
      K'(\tau)&=y^T e^{(T_1-\tau)A} (-A) \cdot B, \; \tau \in [T_0,T_1]\\
      K'(T_1)&=y^T (-A) B .
    \end{align*}
    i-faches Differenzen von $K(\tau)$ nach $\tau$ ergibt
    \begin{align*}
      K^{(i)}(\tau)&=y^T e^{(T_1-\tau)A} \cdot (-1)^i A^i B, \; \tau \in [T_0,T_1]\\
   ...
   \item["`$\Rightarrow$"']
    Die Hinrichtung wird auch per Kontraposition gezeigt, die Beweisidee wird aus der Rückrichtung übernommen. 

    Angenommen, die Kalman Rang Bedigung wäre nicht erfüllt. \\
    Dann existiert ein $y \in \R^n$ mit $y^T A^i B = 0 \; \forall i \in \{0,\dots,n-1\}$.

    Durch Induktion lässt sich zuerst folgende Behauptung beweisen:
    Alle Matrizen $A^m$ mit $m \in \N, m \geq n$ lassen sich durch eine Linearkombination $\sum\limits_{i=0}^{n-1} \alpha^i A^i $ darstellen
    \begin{enumerate}
      \item[(IA)]
      Betrachtet man dass charakteristische Polynom $\chi_A$ der Matrix $A$.
      \[
	\chi_A = X^n+\alpha_{n-1}X^n-1+\dots+\alpha_0
      ...
      A^{m+1} &= A^m \cdot A \overset{IV}{=} (\sum\limits_{i=0}^{n-1} \alpha_i A^i) A = \sum\limits_{i=0}^{n-1} \alpha_i A^{i+1} = \sum\limits_{i=1}^{n} \alpha_{i-1} A^i \\
							&= \sum\limits_{i=1}^{n-1} \alpha_{i-1} +\alpha_{n-1} A^n \overset{IA}{=} \sum\limits_{i=1}^{n-1} \alpha_{i-1} A + \sum\limits_{i=0}^{n-1} \beta_i A^i= \sum\limits_{i=0}^{n-1}\gamma_i A^i
		\end{align*}
		\end{enumerate}
		Es folgt die Behauptung per vollständiger Induktion.
		
		Natürlich lassen sich auch $A^m$ mit $m \in \{0,\dots,n-1\}$ durch $\sum\limits_{i=0}^{n-1} \alpha_i A^i$ darstellen, man wählt dafür einfach $\alpha_i=\begin{cases} 0 & i \neq m \\ 1 & i=m \end{cases}$. \\    
		...
		\[
			K(\tau) = \sum\limits_{i=0}^\infty \frac{\overbrace{K^{(i)}(T_1)}^{=0}}{i!} (\tau-T_1)^i = 0
		\]
		und beachtet $\int_{T_0}^{T_1} |K(\tau)|^2 d\tau = 0$, ergibt sich
		\[ 
			0=\int\limits_{T_0}^{T_1} |B^T e^{(T_1-\tau)A^T} y | ^2 d\tau=\G
		\]
		und damit auch
		\[
		y^T \G y = 0
		\]
		Da $\G$ nach (\ref{Gram'sche Kontrollmatrix:Eigenschaften:symmetrie}) und (\ref{Gram'sche Kontrollmatrix:Eigenschaften:definitheit}) symmetrisch und semipositiv definit  ist, ist durch $(x,y) = x^T \G y$ eine symmetrische semipositive Bilinearform gegeben und auf diese die Cauchy-Schwarzsche Ungleichung anwendbar: Sei $z \in \R^n$
		\[
...
Der Beweis des zeitunabhängigen Falls kann fast vollständig auf den zeitabhängigen Fall übertragen werden.
\begin{Beweis}
...
\]
Indem $z:=R(T_1,\overline t)^T y$ definiert wird, werden die Eigenschaften der Resolvente geschickt ausgenutzt. Man könnte den Beweis auch mit mit dem $K(\tau)=y^t R(T_1,\tau) B(\tau)$ aus dem zeitunabhängigen Fall führen, dies würde aber zu einer schwächeren Version des Satzes führen, in der statt Matrizen abhängig von $\overline t$ bel. $\in [T_0,T_1]$ nur Matrizen zur Zeit $T_1$ ein Kriterium für die Kontrollierbarkeit wäre.\\
 Zu bemerken ist, dass aus (\ref{Resolvente Eigenschaften:3}) folgt, dass $R(T_1,\overline t)$ invertierbar ist und damit dann auch $R(T_1,\overline t)^T$. Da $y$ ungleich $0$ ist, ist auch $z$ nicht $0$.\\
Definiere jetzt
\[
	K(\tau):=z^T R(\overline t,\tau) B(\tau) \overset{\ref{Resolvente Eigenschaften:2}}{=} y^T R(T_1,\overline t) R(\overline t, \tau) B(\tau) = 0 \; \forall \tau \in [T_0,T_1]
\]
...
\]
Da $K \equiv 0$ ist, muss dann auch $K^{(i)} \equiv 0 \; \forall i \in \N$ sein. Mit (\ref{Resolvente Eigenschaften:1}) gilt insbesondere
\[
...
\begin{Bemerkung}
Vergleicht man den zeitabhängigen und zeitunabhängigen Fall gibt es zwei wesentliche Unterschiede. Stattdem im zeitunabhängigen Fall nur eine Kombination "`der ersten"' $n$ Matrizen $A^0B, \dots A^{n-1}B$ zu prüfen war, gibt es im zeitabhängigen Fall unendlich viele Kominationen der Matrizen $B_i(t)$ für jedes $t \in [T_0,T_1]$. Und ein Ausschluss aller dieser Möglichkeiten liefert noch keinerlei Aussage über die Stabilität des Systems.

Ein Übertrag von den ersten $n-1$ Matrizen ist jedoch nicht möglich. \\
Sei zum Beispiel  $T_0=0, T_1=1, \;n=m=1 $ und $ A(t)=0, B(t)=t.$ \\
Dann ist $B_0(t)=t, \;  B_1(t)=\dot B_0(t)-A(t)\cdot B_0(t)= B_0(t)=1$ und $B_i(t)=0 \; \forall i >1$. 
...
Es ist in (\ref{Kalman Rang Bedingung zeitabhängig}) auch nur noch eine Folgerung gegeben ist. Die Hinrichtung aus (\ref{Kalman Rang Bedingung}) lässt sich nicht auf den zeitabhängigen Fall übertragen. Ein entscheidendes Hilfsmittel, nämlich Cayley Hamilton lässt sich nicht auf die $B_i(\overline t)$ anwenden. Tatsächlich ist es so, dass die Kalman Rang Bedingung im zeitabhängigen Fall nur hinreichend und nicht notwendig ist.
\begin{Beispiel}
	Sei $n=2, m=1, A(t)=0$\\
