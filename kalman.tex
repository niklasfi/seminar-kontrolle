...
Ob dann die Kontrollierbarkeit auch nicht von der Zeit abhängt oder welche Rolle die Wahl des Start- und Endpunktes spielen, ist noch von vorneherein klar. 

Betrachte nun das lineare, zeitunabhängige System $\dot{x}=Ax+Bu$ in $[T_0,T_1]$. Ein Kriterium für die Kontrollierbarkeit ist gegeben durch das folgende
...
\begin{Beweis}[zu \ref{Kalman Rang Bedingung}]
 \begin{enumerate}
  \item["`$\Leftarrow$"']
    Da $A$ zeitunabhängig ist können wir die Differentialgleichung leicht lösen und erhalten die Resolvente $R(t_1,t_2)=e^{(t_1-t_2)A} \; \forall  (t_1,t_2) \in [T_0,T_1]^2$ oder $R(t_1,t_2)=e^{(t_1-t_2)A} \; \forall \: (t_1,t_2) \in [T_0,T_1]^2$
    ...
    \end{align*}
		...
		Bewiesen wird die Kontraktion, zu zeigen ist also: Wenn das System nicht kontrollierbar ist, dann ist der $\rang(A^0 \mid A^1B \mid A^2B \mid \dots  \mid A^{n-1}B) \neq n$. 
		
		Sei $\dot x = Ax+Bu$ nicht kontrollierbar in $[T_0,T_1]$. Dann ist $\G$ nicht invertierbar und hat somit einen nicht trivialen Kern. Es exisitert also $y \in \R^n \setminus \{0\}$ mit $\G y=0$
    Demnach gilt
     \[
	y^T \G y = 0 
     \]
   ...
   \item["`$\Rightarrow$"']
	...
	\chi_A = X^n+\alpha_{n-1}X^n-1+\dots+\alpha_0
      ...
      A^{m+1} &= A^m \cdot A \overset{IV}{=} (\sum\limits_{i=0}^{n-1} \alpha_i A^i) A = \sum\limits_{i=0}^{n-1} \alpha_i A^{i+1} = \sum\limits_{i=1}^{n} \alpha_{i-1} A^i \\
		...
		y^T \G y = 0
		\]
		Da $\G$ semipositiv definit und symmetrisch ist nach (\ref{Gram'sche Kontrollmatrix:definitheit}), ist durch $(x,y) = x^T \G y$ eine symmetrische semipositive Bilinearform gegeben und auf diese die Cauchy-Schwarzsche Ungleichung anwendbar: Sei $z \in \R^n$
		\[
...
\]
Indem $z:=R(T_1,\overline t)^T y$ definiert wird, kommt das $\overline t $ zum Einsatz. Zu bemerken ist, dass aus (\ref{Resolvente Eigenschaften:3}) folgt, dass $R(T_1,\overline t)$ invertierbar ist und damit dann auch $R(T_1,\overline t)^T$. Das hießt $z \neq 0$, da $y \neq 0$.\\
Definiere jetzt
\[
