...
\begin{Beweis}[zu \ref{Kalman Rang Bedingung}]
 \begin{enumerate}
  \item["`$\Leftarrow$"']
   ...
   \item["`$\Rightarrow$"']
    Die Hinrichtung wird wieder per Kontraktion gezeigt, die Beweisidee ist dieselbe wie in der Rückrichtung. 
...
\begin{Bemerkung}
Im Gegensatz zum zeitunabhängigen Fall ist jetzt nur noch eine Folgerung gegeben. Und statt vorher nur eine Kombination "der ersten" $n$ Matrizen $A^0B, \dots A^{n-1}B$ zu prüfen war, gibt es jetzt unendlich viele Kominationen der Matrizen $B_i(t)$ für jedes $t \in [T_0,T_1]$. Und ein Ausschluss aller dieser Möglichkeiten liefert noch keinerlei Aussage über die Stabilität des Systems.

Die Forderung ist jedoch notwendig. \\
Sei z.B.  $T_0=0, T_1=1, \;n=m=1 $ und $ A(t)=0, B(t)=t.$ \\
...
\]
Indem $z:=R(T_1,\overline t)^T y$ definiert wird, werden die Eigenschaften der Resolvente geschickt ausgenutzt. Man könnte den Beweis auch mit $K(\tau)=y^t R(T_1,\tau) B(\tau)$ führen, dies würde aber zu einer schwächeren Version des Satzes führen, in der statt $\overline x$ bel. $\in [T_0,T_1]$ nur $T_1$ ein Kriterium für die Kontrollierbarkeit wäre. Zu bemerken ist, dass aus (\ref{Resolvente Eigenschaften:3}) folgt, dass $R(T_1,\overline t)$ invertierbar ist und damit dann auch $R(T_1,\overline t)^T$. Da $y \neq 0$, ist auch $z \neq 0$.\\
Definiere jetzt
\[
