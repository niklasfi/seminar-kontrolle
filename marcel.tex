...
Das hei"st $\text{\ensuremath{\mathcal{R}}(t) ist die Menge der Zustände (oder Punkte), die von \ensuremath{x_{0}=0}}$
...
\end{pmatrix}
$$
Das hei"st, beide Lösungskomponente sind für jede Steuerung $u$ zu
...
\end{align*}
Sei au"serdem $\phi_1 \in \mathbb R^n$ und $\phi : [t_0,t_1] \rightarrow \mathbb R^n $ die Lösung des  adjungierten Systems
\begin{align*}
...
Sei also $x_1 \in \mathcal R$ beliebig, das hei"st es existiert eine Sterung $u^*$, so dass die Lösung $x^*(t)$ sowohl das Anfangswertproblem $$\dot x = A(t)x + B(t)  u^*(t), ~x(t_0)=0$$
