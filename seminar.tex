...
\newenvironment{enumerateRef}{\begin{enumerate}[ref={\thethmc\alph*},label={(\alph*) }]}{\end{enumerate}}
\newenvironment{enumerateBew}{\mbox{}\begin{enumerate}[label={zu (\alph*):}]}{\end{enumerate}}
...
\begin{document}
	...
	\maketitle
	\begin{abstract}
		Dies ist eine Ausarbeitung des Kapitels "Finit-dimensional linear control 
...
\section{Definitionen}
	...
	\subsection*{Eigenschaften}
		...
		\begin{Lemma}
			Für die Lösung eines Linearen Anfangswertproblems \[
				\dot x(t) = A(t) x(t) + b(t) \; \forall t \in [T_0, T_1] 
			\] gilt
		\end{Lemma}
	...
	\begin{Lemma}\label{Resolvente AWP}
		...
		\begin{enumerateRef}
			...
			\item\label{Resolvente AWP:2} $x(t_1) = R(t_1, t_0) \cdot x(t_0) + \int_{t_0}^{t_1} R(t_1,\tau)\cdot b(\tau) d\tau$
			\item\label{Resolvente AWP:3} $x(t) = R(t, T_0)x^0+ \int_{T_0}^t R(t, \tau) \cdot b(\tau)d\tau$
	...
	\begin{Beweis}
		\begin{enumerateBew}
			\item
				 \todo{fehlt!}
			\item
				folgt aus 1. mit $t_0:= T_0$ und $t_1 := t$ 
		\end{enumerateBew}
	\end{Beweis}
\newpage
\printbibliography
